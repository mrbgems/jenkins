...
for ver in $MRUBY_VERSIONS; do
  cd $WORKSPACE
  tar xf $WORKSPACE/../release-downloader/mruby-$ver.tar.gz
  cd mruby-$ver
  MRUBY_CONFIG="$WORKSPACE/mgem_build_config.rb" script -e -c  "./minirake all test" /dev/null || res=1
done
...
mruby_master_xml = REXML::Document.new(`curl '#{base_url}/job/release-downloader/config.xml'`)
...
raise "cannot update mruby-master" unless
  system("curl -X POST --data-binary @- -H #{crumb} -H 'Content-Type: text/xml' '#{base_url}/job/release-downloader/config.xml' < ./tmp.xml")
